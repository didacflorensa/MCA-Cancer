#' MCA Study for Urinary Bladder cancer
#' 
...
cancer.data=read_csv("../../../MCA-Cancer/data/private/analisi-bufeta2-outliers.csv")
...
res.mca.urinary <- MCA(cancer.data.analisi, graph = FALSE)
...
fviz_screeplot(res.mca.urinary, addlabels = TRUE)

#This function create the plot which represent the variable into the dimensions
fviz_mca_var(res.mca.urinary, choice = "mca.cor", 
             repel = TRUE, 
             ggtheme = theme_minimal())

#Correlation between variables
fviz_famd_var(res.mca.urinary, 
              repel = TRUE, # Avoid text overlapping (slow)
              ggtheme = theme_minimal())

# Contribution to the first dimension
fviz_contrib(res.mca.urinary, "var", axes = 1)

# Contribution to the second dimension
fviz_contrib(res.mca.urinary, "var", axes = 2)

#Contribution and correlation in the biplot
fviz_mca_var(res.mca.urinary, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), 
             repel = TRUE, ggtheme = theme_minimal(), labelsize = 7) +
  labs(title = " ", x= paste("Dimension 1 (", round(res.mca.urinary$eig[6], 1),"%)", " "), 
       y = paste("Dimension 2 (", round(res.mca.urinary$eig[7], 1),"%)", " "))+
