...
cancer.data=read_csv("../../../MCA-Cancer/data/private/analisi-prostata2-outliers.csv")

summary(cancer.data)
head(cancer.data)

#Split information and save it into a variable without the Cancer and Gender column 
cancer.data.analisi = cancer.data[, 2:3]

#Apply the MCA technique and save the results into a variable
res.mca.prostate <- MCA(cancer.data.analisi, graph = FALSE)

#Show the results in the consle
res.mca.prostate$eig
res.mca.prostate$var

#This function create the plot which represents the percentage of explained variances by dimensions
fviz_screeplot(res.mca.prostate, addlabels = TRUE)

#This function create the plot which represent the variable into the dimensions
fviz_mca_var(res.mca.prostate, choice = "mca.cor", 
             repel = TRUE, 
             ggtheme = theme_minimal())

#Correlation between variables
fviz_famd_var(res.mca.prostate, 
              repel = TRUE, # Avoid text overlapping (slow)
              ggtheme = theme_minimal())

# Contribution to the first dimension
fviz_contrib(res.mca.prostate, "var", axes = 1)

# Contribution to the second dimension
fviz_contrib(res.mca.prostate, "var", axes = 2)

#Contribution and correlation in the biplot
fviz_mca_var(res.mca.prostate, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), 
             repel = TRUE, ggtheme = theme_minimal(), labelsize = 7) +
  labs(title = " ", x= paste("Dimension 1 (", round(res.mca.prostate$eig[6], 1),"%)", " "), 
       y = paste("Dimension 2 (", round(res.mca.prostate$eig[7], 1),"%)", " "))+
