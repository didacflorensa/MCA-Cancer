...
cancer.data=read_csv("../../../MCA-Cancer/data/private/analisi-mama2-outliers.csv")
...
res.mca.breast <- MCA(cancer.data.analisi, graph = FALSE)
...
fviz_screeplot(res.mca.breast, addlabels = TRUE)

#This function create the plot which represent the variable into the dimensions
fviz_mca_var(res.mca.breast, choice = "mca.cor", 
             repel = TRUE, 
             ggtheme = theme_minimal())

#Correlation between variables
fviz_famd_var(res.mca.breast, 
              repel = TRUE, # Avoid text overlapping (slow)
              ggtheme = theme_minimal())

# Contribution to the first dimension
fviz_contrib(res.mca.breast, "var", axes = 1)

# Contribution to the second dimension
fviz_contrib(res.mca.breast, "var", axes = 2)

#Contribution and correlation in the biplot
fviz_mca_var(res.mca.breast, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), 
             repel = TRUE, ggtheme = theme_minimal(), labelsize = 7) +
  labs(title = " ", x= paste("Dimension 1 (", round(res.mca.breast$eig[6], 1),"%)", " "), 
       y = paste("Dimension 2 (", round(res.mca.breast$eig[7], 1),"%)", " "))+
